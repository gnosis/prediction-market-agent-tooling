...
from prediction_market_agent_tooling.agent.development_tools.prophet_agent_tester import ProphetAgentTester, ProphetTestResult
...
def test_all_models(
    ...
    for agent_index, agent_name in enumerate(agent_names, 1):
        ...
        test_results = tester.test_prophet_agent(dataset, research_agent, prediction_agent)
        ...
        logger.info(f"Completed testing for {agent_name}: {trades_processed} trades processed")
    ...
    for agent_index, agent_name in enumerate(agent_names, 1):
        ...
        if metrics:
            ...
            logger.info(f"{agent_index}. {agent_name} ({trades_count} trades): No metrics available")
...
def main(
    ...
    max_trades: int = typer.Option(3000, "--max-trades", help="Maximum number of trades to test per agent"),
    delay: float = typer.Option(2.0, "--delay", help="Delay in seconds between processing each trade"),
    ...
    research: bool = typer.Option(False, "--research", help="Include research generation"),
    prediction: bool = typer.Option(False, "--prediction", help="Include prediction generation"),
) -> None:
