...
from gnosis.safe import ProxyFactory, addresses
...
from prediction_market_agent_tooling.deploy.safe.utils import send_tx


def deploy_safe_with_create2(
    client: EthereumClient, account: LocalAccount, signers: list[str], threshold: int
) -> ChecksumAddress:
    w3 = client.w3
    ...
    safe_address: ChecksumAddress = builder.calculate_create2_address(
        setup_data, salt_nonce
    )
    ...
    proxy_factory = ProxyFactory(
        Web3.to_checksum_address(PROXY_FACTORY_ADDRESS), client
    )
