# prediction_market_agent_tooling/performance/slack/alert_on_slack.py
...
class PerformanceAlertAgent(DeployableAgent):
    ...
    def _get_performance_data_from_dune(self, market_type: MarketType) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        performance_query = self._querry(PERFORMANCE_QUERY_ID, "Agents Performance For last week", "Last Month", "PMA")
        precision_query = self._querry(PRECISION_QUERY_ID, "Agents Precision For last week", "Last Month", "PMA")

        performance_result = self._get_latest_result(performance_query.id) or self._process_new_querry(performance_query)
        precision_result = self._get_latest_result(precision_query.id) or self._process_new_querry(precision_query)   

        return performance_result, precision_result

    def _wait_for_dune_execution(self, query_id: int):
        start_time = time.time()
        while time.time() < start_time + MAX_WAIT_MINUTES * 60:
            res = self._get_latest_result(query_id, 1)
            if res:
                return res
            time.sleep(POLL_INTERVAL_SECONDS)
        res = self._get_latest_result(query_id, 24)
        if res:
            return res
        raise RuntimeError(f"No result available for query {query_id}")

    def _process_new_querry(self, query: QueryBase):
        try:
            return self.dune.run_query(query, ping_frequency=POLL_INTERVAL_SECONDS)
        except Exception:
            logging.error(f"Querry timed out {query.query_id}, waiting for {MAX_WAIT_MINUTES} minutes for it to finish")
            return self._wait_for_dune_execution(query.query_id)

    def _get_latest_result(self, query_id: int):
        try:
            return self.dune.get_latest_result(query_id, max_age_hours=8)
        except Exception:
            return None

    def _querry(self, query_id: int, name: str, range_value: str, ranking_value: str) -> QueryBase:
        return QueryBase(
            name=name,
            query_id=query_id,
            params=[
    ...
    def _weekly_cumulative_profit(self, df_performance: pd.DataFrame) -> pd.DataFrame:
        df_performance["block_date"] = pd.to_datetime(df_performance["block_date"]).dt.tz_localize(None)
        df_performance = df_performance.sort_values(["label", "block_date"]).reset_index(drop=True)
        ...
        for _, label_group in df_performance.groupby("label"):
            ...
            picked_df = label_group[label_group["date_only"].isin(set(picked.date))].copy()

            if len(picked_df) > 0:
                picked_df = picked_df.drop("date_only", axis=1)
                picked_df = picked_df[["label", "block_date", "cumulative_return"]].copy()
        ...
        result["cumulative_return"] = pd.to_numeric(result["cumulative_return"], errors="coerce")
        result["block_date"] = pd.to_datetime(result["block_date"], utc=True).dt.normalize()
    ...
    def _prepare_precision_daily(self, df_precision: pd.DataFrame) -> pd.DataFrame:
        ...
        df_prec["pct_correct"] = pd.to_numeric(df_prec.get("pct_correct"), errors="coerce")
        df_prec["cnt_resolved"] = pd.to_numeric(df_prec.get("cnt_resolved"), errors="coerce")
        ...
        result = df_prec[["label", "date", "pct_correct", "pct_correct_7d", "cnt_resolved"]]
        result = (
...
def _get_performance_report(self, accuracy_df: pd.DataFrame, profit_df: pd.DataFrame, count_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    weekly_combined = weekly_combined.drop(columns=["pct_correct", "pct_correct_7d", "cnt_resolved"])
    weekly_combined["sequence"] = weekly_combined.groupby("label").cumcount() + 1

    #Format columns
    weekly_combined["profit_formatted"] = weekly_combined["cumulative_return"].apply(format_profit)
    weekly_combined["daily_accuracy_formatted"] = weekly_combined["daily_pct_correct"].apply(format_daily_accuracy)
    weekly_combined["avg_accuracy_formatted"] = weekly_combined["precision_7d_avg"].apply(format_avg_accuracy)
    weekly_combined["count_formatted"] = weekly_combined["daily_cnt_resolved"].apply(format_count)
    ...
    def _format_agent_block(self, label: str, g: pd.DataFrame) -> str:
        ...
        for _, r in g.iterrows():
            date_str = r["block_date"].strftime("%Y-%m-%d") if pd.notna(r["block_date"]) else "N/A"

            cur_profit_num = float(r["cumulative_return"]) if pd.notna(r["cumulative_return"]) else None
            trend = self._trend_icon(prev_profit_num, cur_profit_num)
            profit_str = f"{trend}{r['profit_formatted']}" if trend else f"{r['profit_formatted']}"
    ...
    def _prepare_report_string(self, combined_report: pd.DataFrame) -> str:
        combined_report["block_date"] = pd.to_datetime(combined_report["block_date"], utc=True, errors="coerce")
        combined_report = combined_report.sort_values(["label", "block_date", "sequence"]).reset_index(drop=True)
        ...
        body= [
            for label, g in combined_report.groupby("label", sort=False):
                parts.append(self._format_agent_block(label, g))
        ]
...
def _send_report_to_slack(self, header: str, body: str, footer: str) -> None:
    ...
    if body:
        agent_blocks = body.split('\n\n')  # Each agent block is separated by empty line
        current_message = ""
        ...
        for block in agent_blocks:
            ...
            if current_message and len(current_message) + len(block_with_spacing) > SAFE_SLACK_MESSAGE_LENGTH:
                requests.post(self.slack_webhook, json={"text": current_message.rstrip()}, timeout=10)
    ...
    def run(self, market_type: MarketType) -> None:
        accuracy_df, profit_df, count_df = self.get_performance_data_from_dune(market_type)
        combined_report = self.get_performance_report(accuracy_df, profit_df, count_df)
