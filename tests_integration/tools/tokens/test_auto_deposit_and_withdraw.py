...
def test_auto_deposit_and_withdraw(
    ...
    to_deposit_as_keeping_token_wei = to_deposit_as_keeping_token.as_wei

    # and in the other token
    to_deposit_as_token_2 = get_usd_in_token(
        to_deposit,
        collateral_token_contract.address,
    )
    to_deposit_as_token_2_wei = to_deposit_as_token_2.as_wei
    ...
    initial_balance_of_token_2_in_usd = get_token_in_usd(
        collateral_token_contract.balance_of_in_tokens(api_keys.bet_from_address),
        collateral_token_contract.address,
    )
    total_amount_in_usd = initial_balance_of_token_2_in_usd + to_deposit
    # And how much we expect to have in token_2 after the deposit.
    total_amount_in_token_2 = get_usd_in_token(
        total_amount_in_usd,
        collateral_token_contract.address,
    )
    ...
    assert after_deposit_balance_main_token == 0 or np.isclose(
        ...
        (before_deposit_balance_main_token - to_deposit_as_keeping_token_wei).value,
        rtol=rtol,
    ), "Should be 0 if it was deposited and then used, or should be less by the amount we sold."
    assert np.isclose(
        after_deposit_balance_token_2.value,
        (before_deposit_balance_token_2 + to_deposit_as_token_2_wei).value,
        rtol=rtol,
    ), "Should be more by the amount we bought."
    assert np.isclose(
        total_amount_in_token_2.value, after_deposit_balance_token_2.value, rtol=rtol
    ), "Should be equal, because we bought exactly as much as we expected in the beginning."

    auto_withdraw_collateral_token(
        collateral_token_contract,
        amount_wei=to_deposit_as_token_2_wei,
        api_keys=api_keys,
    )
    ...
    assert np.isclose(
        ...
        (after_deposit_balance_main_token + to_deposit_as_keeping_token_wei).value,
        rtol=rtol,
    ), "Should be more by the amount we bought."
    assert np.isclose(
        after_withdraw_balance_token_2.value,
        (after_deposit_balance_token_2 - to_deposit_as_token_2_wei).value,
        rtol=rtol,
